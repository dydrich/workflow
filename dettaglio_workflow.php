...
$sel_gruppi = "SELECT * FROM gruppi WHERE gid BETWEEN 2 AND 5";
$res_gruppi = $db->executeQuery($sel_gruppi);

$sel_step = "SELECT * FROM w_step";
$res_step = $db->executeQuery($sel_step);

if(isset($_REQUEST['id']) && ($_REQUEST['id'] != 0)){
	$sel_flow = "SELECT * FROM w_workflow WHERE id_workflow = ".$_REQUEST['id'];
	//print $sel_flow;
	try{
	...
	$s_step = "SELECT * FROM w_step WHERE id_step IN (".$flow['codice_step'].")";
	//print $s_step;
	try{
    ...
    $_i = $_REQUEST['id'];
    $sel_count_groups = "SELECT COUNT(*) AS ct FROM gruppi WHERE permessi&".$flow['gruppi'];
    try{
