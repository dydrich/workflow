...
if(isset($_REQUEST['action']) && ($_REQUEST['action'] == "update")){
	...
	$update = "UPDATE w_uffici SET nome = '".$value."' WHERE id_ufficio = ".$id_ufficio;
	try{
	...
	$sel_max = "SELECT MAX(codice_permessi) AS max FROM w_uffici";
	$_max = $db->executeCount($sel_max);
	$max = $_max*2;
	if($max == 0)
		$max = 1;
	$nome = $_REQUEST['nome'];
	$insert = "INSERT INTO w_uffici (nome, codice_permessi) VALUES ('".$nome."', $max)";
	try{
	...
	$id = $_REQUEST['id'];
	$delete = "DELETE FROM w_uffici WHERE id_ufficio = $id";
	try{
...
$sel_uffici = "SELECT * FROM w_uffici ORDER BY id_ufficio DESC";
